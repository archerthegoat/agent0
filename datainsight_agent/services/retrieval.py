...
class RetrievalService:
	...
	- Local HNSW vector index (always available if built)
	- Local SQLite graph (always available if built)
	- Placeholders for Elasticsearch/Milvus/Neo4j (feature flags in settings)
	"""

	def __init__(self, vector_store: Optional[LocalHNSWVectorStore] = None, local_graph: Optional[LocalGraphClient] = None) -> None:
	...
	def recall_by_embedding(self, embedding: List[float], top_k: int = 5) -> List[KBEntity]:
		...
		if self.local_graph:
			rows = self.local_graph.find_by_concepts(ids, limit=top_k)
			for row in rows:
				try:
	...
	def hybrid_knowledge_retriever(self, concepts: List[str], top_k: int = 5) -> List[KBEntity]:
		...
		If ES/Milvus/Neo4j feature flags are enabled in settings and corresponding
