...
def month_list(year: int) -> List[str]:
...
def gen_rows(n: int, months: List[str]) -> List[Dict[str, Any]]:
	...
	for _ in range(n):
		uid = f"u{random.randint(1, 400000)}"
		rows.append({
			"user_id": uid,
...
def main() -> None:
	...
	months = month_list(args.year)
	rows = gen_rows(args.rows, months)
	insert_rows(args.db, rows)
	print(f"Inserted rows: {len(rows)} for year {args.year} into {args.db}")
